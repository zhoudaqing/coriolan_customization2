$(document).ready(function() {
    //cache check
    console.log('CACHE change_fav 99');
   // var declarations
   ...
   var current_url=document.URL; //used to generate product link
   var product_img_container="div.ty-product-block__img-wrapper.span10";
   var fav_block_id='#dropdown_279';
    $('.ty-add-to-wish > a').on('click', function() { //user click on 'add product to wishlist'
       if (!$('div.ls_mid_myaccount').has('a[href="'+current_url+'"]').length) { //product isn't already added
        ...
        //get the id required to delete the product from wishlist
            ...
            request1.done(function (msg) {
               /* footerFavId2=msg; */
                console.log('getCartId msg:'+msg); 
            });
                //check to see if a main image exist(to prevent thumbs from being loaded in carousel by the selector below and thrn lost on refresh)
              if(!$(product_img_container).find('span.ty-no-image').length) { //imagine exist, append it to carousel
               //obtain the image
               ...
               fav_product_img=$('<div>').append($(fav_product_img).clone()).html(); //wrap it in a div then get the html of the div for image markup
                ...
                '<a href="'+current_url+'">'+fav_product_img+'</a></div>';
