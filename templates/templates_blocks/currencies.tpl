    ...
    {if $dropdown_limit > 0 && $currencies|count <= $dropdown_limit}
        <div class="ty-currencies hidden-phone hidden-tablet">
            ...
            {foreach from=$currencies key=code item=currency}               
                <a href="{$config.current_url|fn_link_attach:"currency=`$code`"|fn_url}" class="ty-currencies__item {if $secondary_currency == $code}ty-currencies__active{/if}">{if $format == "name"}{$currency.description}&nbsp;({$currency.symbol nofilter}){else}{$currency.description}&nbsp;({$currency.symbol nofilter}){/if}</a>
            {/foreach}
        </div>
