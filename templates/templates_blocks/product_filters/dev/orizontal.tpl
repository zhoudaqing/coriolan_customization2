...
{foreach from=$items item="filter" name="filters"}
    {if $filter.slider || $filter.selected_ranges || $filter.ranges}
        ...
        <li class="ty-menu__item cm-menu-item-responsive ">
            <div id="sw_content_{$filter_uid}" class="ty-product-filters__switch cm-combination-filter_{$filter_uid}">
...
{/foreach}
{*
    {if $colorOptionVariants|@count gt 0}
    <li class="ty-menu__item cm-menu-item-responsive ">
        <div id="sw_content_test" class="ty-product-filters__switch cm-combination-filter_{$filter_uid}">
            <span class="ty-product-filters__title">{__('color')}</span>
            <i class="ty-product-filters__switch-down ty-icon-down-open"></i>
            <i class="ty-product-filters__switch-right ty-icon-up-open"></i>
        </div>
        <ul id="content_test" class="ty-product-filters" style="display:none;">
            <li class="ty-product-filters__item-more">
                <ul id="ranges_test">
                    <li class="ty-product-filters__group">
                        <select onchange="ls_filter_by_color(this.value);">
                            <option value="">- select -</option>
                            {foreach from=$colorOptionVariants item="variant" name="variants"}
                            <option value="{$variant.option_id}_{$variant.variant_id}">{$variant.variant_name}</option>
                            {/foreach}
                        </select>
                    </li>
                </ul>
    ...
    <li class="ty-menu__item cm-menu-item-responsive ">
        <div class="ty-product-filters__tools clearfix">
            ...
            {if $smarty.capture.has_selected}
                <a href="{if $smarty.request.category_id}{assign var="use_ajax" value=true}{"categories.view?category_id=`$smarty.request.category_id`"|fn_url}{else}{assign var="use_ajax" value=false}{""|fn_url}{/if}" rel="nofollow" class="ty-product-filters__reset-button{if $allow_ajax && $use_ajax}" data-ca-scroll=".cm-pagination-container" data-ca-target-id="{$ajax_div_ids}{/if}"><i class="ty-product-filters__reset-icon ty-icon-cw"></i> <span>{__("reset")}</span></a>
            {/if}
        </div>
    </li>
</ul>
