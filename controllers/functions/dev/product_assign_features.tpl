...
{foreach from=$product_features item=feature key="feature_id"}
    ...
    {if $feature.feature_type != "G"}
        <div class="control-group">
            ...
            {elseif $feature.feature_type == "M" && $feature.feature_variants_images==0}
                ...
                {/if}
            {elseif $feature.feature_type == "M" && $feature.feature_variants_images==1}
                
                <input type="hidden" name="product_data[product_features][{$feature_id}]" value="" />
                <table style="width:100%">
                    {foreach from=$feature.variants item="var"}
                        <tr>
                            <td><label for="variant_{$var.variant_id}" class="checkbox" style="display:inline-block;"><input type="checkbox" id="variant_{$var.variant_id}" name="product_data[product_features][{$feature_id}][{$var.variant_id}]" {if $var.selected}checked="checked"{/if} value="{$var.variant_id}" />{$var.variant}</label></td>
                            <td><a href="#" onclick="show_product_variant_details({$var.variant_id});return false;">{__("details")}</a><br/></td>
                        </tr>
                        <tr id="variant_details_{$var.variant_id}" class="hidden">
                            <td colspan="2">
                                <div>
                                    <div class="control-group">
                                        <label class="control-label" for="elm_image_{$var.variant_id}">{__("image")}</label>
                                        <div class="controls">
                                            {$var.product_image_pairs_extra|var_dump}
                                            {include file="common/attach_images.tpl" image_name="p_feature_var_extra" image_object_id=$var.variant_id hide_titles=true image_key=$var.variant_id no_detailed=true image_object_type="p_feature_var_extra" image_type="M" image_pair=$var.product_image_pairs_extra prefix=$id}
                                        </div>
                                    </div>
                                </div>
                            </td>
                ...
                {*{foreach from=$feature.variants item="var"}
                    <label for="variant_{$var.variant_id}" class="checkbox" style="display:inline-block;"><input type="checkbox" id="variant_{$var.variant_id}" name="product_data[product_features][{$feature_id}][{$var.variant_id}]" {if $var.selected}checked="checked"{/if} value="{$var.variant_id}" />{$var.variant}</label>
                    <a href="#" onclick="return false;">details</a><br/>
                {/foreach}*}
                {*{if $allow_enter_variant}
                <input type="text" name="product_data[add_new_variant][{$feature.feature_id}][variant]" id="feature_{$feature_id}" placeholder="{__("enter_other")}" />
                {/if}*}
