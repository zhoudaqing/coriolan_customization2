...
//    echo "<br>combination hash: $combination_hash, order amount: {$product['order_amount']} product_id: {$product['product_id']}, product db hash {$product['ls_db_hash']} , ls_main_product_info tracking without options stock: " . $product['ls_main_product_info']['amount']."; main product tracking with options stock: {$product['inventory_amount']};first linked product total order amount : {$product['ls_get_product_variants'][0]['total_order_amount']}; first linked product stock amount: {$product['ls_get_product_variants'][0]['linked_product_amount']} <br>";
   // echo "avail since= {$product['avail_since']}, order_procesing={$product['ls_order_processing']}, comm period={$product['comm_period']}" ;
    ...
    $ls_option_linked = 'Nu';
    if (empty($product['ls_get_product_variants'])) { //product has no variants
...
//check the product tracking
        if ($product['tracking'] === 'O') { //product tracking with options
         //  echo '<br> no variants, tracking O </br>';      
            ...
            if ($product['tracking'] === 'B') {  //product tracking without options
...
//add the variants only if the product is tracked with options
...
function fn_linked_products_in_cart_amount(&$cart_products,$return_id) {
    foreach($cart_products as $hash0=>$noVariants_product) {
        if (!isset($noVariants_product['order_amount'])) { //checkout estimation
            $noVariants_product['order_amount'] = $noVariants_product['amount'];
        }
            ...
            foreach($cart_products as $hash1=>$withVariants_product) {
                ...
                foreach ($withVariants_product['ls_get_product_variants'] as $k1 => $linked_product) {
                    if ($linked_product['linked_product_id'] == $noVariants_product['product_id']) {
                        //correct the amounts
                        $cart_products[$hash1]['ls_get_product_variants'][$k1]['total_order_amount']+=$noVariants_product['order_amount'];
...
function fn_ls_get_minicart_options($product_options, $lang_code = CART_LANGUAGE) {
    foreach($product_options as $option_id=>$variant_id) {
        $ls_minicart_options[]= db_get_array("SELECT  b.option_id, g.option_name, c.variant_id, d.variant_name, g.lang_code
...
LEFT JOIN cscart_product_option_variants_descriptions AS d ON (c.variant_id = d.variant_id AND g.lang_code=d.lang_code)
WHERE  d.variant_id = ?i
AND b.option_id =?i ORDER BY g.lang_code",$variant_id,$option_id);
    } 
    return $ls_minicart_options;
}
//return categories data coresponding with a keyword
function fn_ls_autocomplete_categories($keyword, $max_results = 3, $lang_code = CART_LANGUAGE) {
    $keyword=$keyword.'%';
     $autocomplete_categories[]= db_get_array("SELECT cscart_category_descriptions.category AS category, cscart_category_descriptions.category_id AS cid FROM cscart_category_descriptions JOIN cscart_categories "
...
//check if the searchword is a category name
function fn_ls_verify_category_name($search_word) {
...
//generate html for a product in the minicart
...
function fn_is_product_in_cart($page_product,&$cart_products,&$template_product) {
    ...
    foreach($cart_products as $hash=>$cart_product) {
     ...
     //       echo "page product order amount when it is in cart={$cart_products[$hash]['order_amount']}";
...
//wishlist products for footer carousel
...
function fn_ls_wishlist_products_number() {
